...
function extractData(files) {
	...
	var Cell = function(cell) {
        ...
        return {
            row: parseInt(cell[1]),
            column: colToInt(cell[0])
        };
	};

	var d = sheet.get('//a:dimension/@ref', ns).value().split(':');
    
	d = _.map(d, function(v) { return Cell(v); });
    ...
    /*
    var sht = sheet.toString().replace(/\r?\n/g, ' '),
        sheetRegexp = /<sheetData(?:\s.*?)?>(.*?)<\/sheetData>/ig,
        cellsRegexp = /(?:.*?<row\s.*?>.*?(<c\s.*?>.*?<\/c>).*?<\/row>)* /ig,
        clls = [], y;
    
    sht = sheetRegexp.exec(sht)[1];
    
    while(y = cellsRegexp.exec(sht)) {
        clls.push(y[1]);
    }
    
    console.log('CELL', clls);
    
    require('fs').writeFile('sheet', new Buffer(sht), console.log);
    */
    ...
    var len = cells.length,
        _cll, r, type, value, cll;
    
    var strs = strings.toString(),
        vals = [], dt,
        regexp = /<t>(.*?)<\/t>/ig;
    
    //require('fs').writeFile('xlsx', new Buffer(strs), console.log);
    for(var i = 0; i < len; i++) {
        dt = regexp.exec(strs);
        _cll = cells[i];
		...
		r = _cll.attr('r').value();
        cll = Cell(r);
		data[cll.row - d[0].row][cll.column - d[0].column] = dt?dt[1]:'';
        continue;
        type = (_cll.attr('t') || na).value(),
        value = ( _cll.get('a:v', ns) || na ).text();

    //console.log('//a:si[' + (parseInt(value) + 1) + ']//a:t', strings.toString());
